...
for file in `find static/uploads -type f -maxdepth 1 -iname '*jpg' -or -iname '*png'`; do
  if test "`identify $file | awk '{print $3}' | cut -dx -f1`" -gt 1500; then
    convert "$file" -resize '1500x1500>' -verbose "$file"
  fi
done

for file in `find static/uploads -type f -mindepth 2 -iname '*jpg' -or -iname '*png'`; do
